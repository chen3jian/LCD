...
class TagLib {
    ...
    public function parseXmlAttr($attr,$tag) {
        ...
        if(!$xml) {
            throw new \Exception('XML标签语法错误 : '.$attr);
        }
    ...
    public function parseCondition($condition) {
        ...
        if(false !== strpos($condition, '$Think'))
            $condition      =   preg_replace_callback('/(\$Think.*?)\s/is', array($this, 'parseLcdVar'), $condition);
        return $condition;
    }
    ...
    public function autoBuildVar($name) {
        if('Lcd.' == substr($name,0,7)){
            // 特殊变量
            return $this->parseLcdVar($name);
        }elseif(strpos($name,'.')) {
            $vars = explode('.',$name);
    ...
    public function parseLcdVar($varStr){
        ...
        if(count($vars)>=3){
            ...
            switch($vars[1]){
                ...
                case 'LANG':       $parseStr = 'Lang::read("'.$vars[2].'")';break;
                case 'CONFIG':    $parseStr = 'Conf::read("'.$vars[2].'")';break;
            }
        }else if(count($vars)==2){
            switch($vars[1]){
                case 'NOW':       $parseStr = "date('Y-m-d g:i a',time())";break;
                case 'VERSION':  $parseStr = 'Lcd_VERSION';break;
